...
- **Plan marker tap → menu.** "add another 2 buttons here that is go to pindrop location and view job note. no more press to go to itp, but press goes to this menu." A plain tap on a coded marker now opens the same action sheet a long press / right-click did: **Open ITP · Open rework · Go to pin-drop location · View job note · Cancel**. A legacy marker with no joinery code still goes straight to its room list. *Go to pin-drop location* reads the item's `deliveryLocationPin` from UTZLINE Delivery ITP's own checklist (the data the "Delivery locations" layer already draws), switches that layer on, pans/zooms the plan to the pin and pulses it; with no pin saved it says so in a toast. *View job note* opens the same read-only job-note list the checklist screen's button opens, without leaving the plan.
- **No text selection.** "is there a way to stop chrome from selecting the text on the page. we just want to pan, zoom and load the menu buttons." `user-select: none` / `-webkit-touch-callout: none` on the app chrome and the whole plan SVG (marker text also ignores pointer events); real text inputs stay selectable. `touch-action: none` on the plan SVG.
- **Save & exit.** "save on the itp should be save and exit, no need for export pdf or delete this item buttons, if an itp is complete and saved, it then creates the pdf. this then has a pop up that is return to room or return to floor plan or return to start." The checklist's buttons are now **Save & exit** and **View job note**. Save & exit runs the same save as before (which already creates the PDF the moment the checklist reaches Signed off — the v23 rule, unchanged) and then asks **Return to room / Return to floor plan / Return to start** (Projects screen). "Return to floor plan" re-renders the current level's plan from memory (no re-read). The Back link and its unsaved-changes dialog are unchanged. The manual Export PDF and Delete this item buttons are gone (`exportChecklistPdf` remains, used only by the sign-off auto-export).
- **Locked once complete.** "once an itp is saved and exported (complete), that itp page becomes locked (read only)." A checklist that opens signed off (both signatures, no "No" — the same rule that creates the PDF) shows a 🔒 banner, has every answer button, comment, note, signature block and photo control disabled (photo remove buttons hidden), and its action button reads **Exit** (no write; same where-to-next choice). A *flagged* checklist (signatures present but a "No" outstanding) stays editable — it still needs fixing.
- **Rooms list alphabetical.** "the rooms list page needs to be in alphabetical order" — a flat project's rooms were shown in the order they were added in UTZLINE Projects. Natural, case-insensitive sort (AH.002 before AH.010), both project shapes.

Tests: new `run_room_list_alpha_and_marker_menu.js` (pin-drop pan lands the pin dead-centre, layer switched on, pulse; no-pin toast; job-note dialog from the plan; Andrew's own 14 room names sorted; user-select off on plan/rows, on in inputs). Every test that pressed Save then kept editing, pressed the old Export PDF, deleted an item, or tapped a marker expecting the checklist was reworked to the new flow — and gained checks for the after-save dialog, the lock, and "Exit" writing nothing. Sign-off scenarios that flagged a "No" *after* sign-off (now impossible on a locked sheet) were reordered to flag before the second signature. Full suite green (10 in `pdftest-itp/`, 9 in `pdftest-projects/`). `service-worker.js` cache → `utzline-itp-cache-v31`.

**v30 (2026-09-25):** performance round 5 — from Andrew's second Timings screenshot (v29, same level): `marker ticks resolved 11566 ms (cancelled, 60 markers, 12 from index, 1 folders listed, 0 files read)` and `status badges resolved 11095 ms (cancelled, 50 of 60 items have history)`; tap → checklist 1.45 s, save 1.2 s (3 photos), back 0.5–1.0 s. Andrew: "getting faster". The self-describing labels did their job: with only one folder listed and nothing read, what was left in the ✓/✕ pass was the **index reads themselves** — one small file per room, twelve rooms; and in the badge pass, the **fifty items that have history each needing their own folder listed and events read**. On that tablet every round trip is hundreds of ms, so both add up to ~11 s regardless of how small each read is.

1. **One index file per level** (`Install ITP Index/<Level>.json`; flat: same name under `Project Saves/UTZLINE ITP/Install ITP Index/`). Entries are keyed `<Room>/<checklist filename>` so a legacy "K-01.json" in two rooms can't collide. The plan reads it once; the Items list reads it once. Writes are serialized through one promise chain so concurrent room backfills can't lose each other. The old per-room files are simply no longer read (derived cache — an empty one is just a cold rebuild). "Not started" is now remembered for legacy projects too (previously flat only) — a legacy level with one never-started marker per room was re-listing every room on every visit.
2. **Badge cache, same pattern**: `Install ITP Index/<Level> - Badges.json`, `{key: {icon, at}}`. A warm plan reads it once. Only keys never seen, or older than **4 hours**, go through the real fold (one listing of `Joinery Status` filtered to just those keys, then only the matched items' small folders, event files read together rather than one after another). This app's own status advances (`setJoineryStatusForward`, e.g. a sign-off here) patch the entry immediately. **Stated limit:** a status advanced by *another* app (Manufacture ITP, Machine Schedule, Delivery ITP) can show its previous badge here for up to 4 hours, because those apps don't know about this cache. Purely derived, never a source of truth.
3. `PLAN_STATUS_CONCURRENCY` 3 → 6: the tablet's numbers say each call is latency-bound, not throughput-bound.

Timing labels now include "N from cache". Under the Android-like mock (`run_round4_android_like_fs.js`, 12 rooms / 60 markers / 400 status keys): v28 cold 13.6 s → v30 cold 4.1 s (3.1 s file-layer); **warm pass 74 ms of file-layer time with zero folders listed** (v29: 270 ms; v28: 13.3 s); a warm visit is three reads total (plan file, level index, badge cache). New checks: an over-age badge entry is re-verified and corrected while the other 59 come from cache; opening a freshly signed-off item advances its status and patches its badge entry straight away. Legacy test updated for the per-level file and remembered "not started"; auto-export test given a one-second gap before its manual export (the app got fast enough for two exports to land in the same to-the-second filename). Full suite green (9 + 9). `service-worker.js` cache → `utzline-itp-cache-v30`.

**v29 (2026-09-25):** performance round 4 — driven by real numbers for the first time. Andrew's Timings screenshot from the tablet on v28 (Mt Barker Hospital, legacy project, 60 markers on the level): level → plan 435 ms (fine); **marker ticks 14.5 s and status badges 14.8 s** (both still running when he tapped); plan tap → checklist 1.5 s for an *empty* checklist; save 1.6 s with 2 photos; back 0.6 s. Two conclusions, both now built in and both explained in a comment above `loadLevelPlanItpStatuses`:

1. **On Android, fewer calls beats smaller calls.** A lookup *by name* into a content-provider folder isn't indexed — the provider scans the folder — so 60 named lookups into a project-wide folder cost far more than one listing of it. v28's badge pass did exactly that (one `getDirectoryHandle(key)` per marker into `Joinery Status`, which holds every item in the whole building) and it was the single biggest cost on the device. Now: **one** cancellable listing of that folder, matched against the level's keys in memory; only matched items' small event folders are read, straight from the handles the listing returned. Keys with no folder resolve to "no badge" live (clearing a stale remembered one).
2. **Don't list what the index already answers.** v28 still listed every room's checklist folder(s) on every plan open even when the index already covered every marker in the room. Now a room's folder is listed only if at least one of its markers is something the index has never seen; a flat project's shared folder is listed at most once per pass and only if needed, and reads use the handles from that listing rather than a second named lookup.

Also trimmed from the tap path: the room folder handle (`levelHandle.getDirectoryHandle(room)`) and the old-`itp`-folder migration check are cached per session (a project with no old folder is discovered once, not re-probed per room). The Timings lines now say what the passes actually did, e.g. "60 markers, 60 from index, 0 folders listed, 0 files read" / "3 of 60 items have history" — so the next screenshot is self-explaining.

**Testing — and a correction to how this app is tested.** The mocks used through rounds 1–3 charged a flat few milliseconds per call with O(1) lookups by name: a desktop file system. That is why each round looked finished here and wasn't on the tablet. New `run_round4_android_like_fs.js` charges 4 ms + 0.5 ms *per sibling* for a named lookup, 2 ms per iterated entry, 10 ms per read/write, on a fixture shaped like the real job (legacy, 12 rooms, 60 markers, 400 status keys project-wide). **Against v28 it reproduces Andrew's number: 13.6 s cold background pass** (his: 14.5 s). Against v29: 4.0 s cold, 0.27 s of file-layer time warm, zero named lookups into any big folder, one listing of `Joinery Status`, all 60 markers and badges correct, 4 named lookups on a warm marker tap. `run_round3_back_and_dir_cache.js` updated to wait on the badge itself rather than a fixed pause. Full suite (9 files in `pdftest-itp/`, 9 Install ITP tests in `pdftest-projects/`) green. `service-worker.js` cache → `utzline-itp-cache-v29`.

**v28 (2026-09-25):** performance round 3 + three-state markers + on-device timings. Andrew, after v27: "still lagging on my tablet, getting better but still an issue, mostly from the plan still, but also pressing back from an itp is slow." Four changes:

1. **Directory-handle cache.** Every fixed folder walk (`Project Saves → UTZLINE ITP → Install ITP`, the PDF folder, the index folder, `Joinery Status`, the legacy `itp-install/<Level>/<Room>` path) is now memoized per project. On Android each `getDirectoryHandle` is a real cross-process call, and opening an item from the plan was re-walking ~10 of them before touching the checklist itself, then the same again on the way back. Cleared on project switch; an entry is dropped if an operation through it fails, so a renamed folder just re-resolves. Only successful lookups are cached (a folder another app hasn't created yet is never remembered as missing).
2. **"Not started" is remembered too (flat projects).** The item-status index now stores `{missing:true}` for an item confirmed to have no checklist file, so neither the Items list nor the plan probes for never-started items on a warm visit (that used to be one failing lookup per never-started item, every time). Self-heals the same way as any entry: replaced by real data when the file is created here, or when the item is next opened here after another device created it.
3. **Plan loader no longer lists whole folders.** v27 walked the entire shared checklist folder and the entire `Joinery Status` folder (both project-wide, hundreds of entries on a big job, not interruptible) on every plan open. Now: the index answers everything it has seen, only never-seen items get one probe each (bounded by this level's markers), and status badges probe each key directly off the cached root. Every remaining directory walk checks for cancellation between entries.
4. **Back from a checklist shows the Items list immediately**, dimmed, and refreshes it behind the save — previously the checklist stayed on screen (looking frozen) until the full write (all photo data) and the list rebuild had both finished. Nothing about what gets saved or when changed. Opening from a plan marker now shows "Loading…" in that list rather than another room's stale rows.

Also: **three-state markers** (Andrew: "red x if not complete, green tick if finished and a blue x for started but not finished" / "ok lets just do the blue additional") — red ✕ not started, blue ✕ started but not signed off, green ✓ signed off; free, from the index's own `answered`/`bothSigned` fields. **Last-known marker snapshot** (Andrew: "should the icons be cached so that when you open it they are already there, then it updates in the background?" — yes): the last resolved colour/tick and status badge for every marker on a level is kept on the device itself (the app's own IndexedDB, no file access) and painted the instant the plan appears; the real loaders then correct it in the background. A save on this device writes through to it immediately. Purely a first-paint hint — nothing else reads it. And a **"⏱ Timings" button on the Projects screen**: the app now times its own key steps (level → plan on screen, plan tap → checklist on screen, list tap → checklist, save, sign-off PDF export, back → list refreshed) and shows the last 60, newest first, so a slow step on the real tablet can be pointed at rather than guessed. New test `run_round3_back_and_dir_cache.js` (flat, slow serialized mock file layer): zero fixed-path folder lookups on a warm marker tap, zero file probes on a warm Items list, Items screen visible within a frame of "Save & leave" with the edit landing on disk before the refresh completes, "Loading…" behind a marker-opened checklist; with the file layer throttled to 250 ms/op every marker is coloured within 1 ms of the plan appearing (snapshot), an item started on this device paints blue straight away (write-through), a change that arrived via the shared index is corrected in the background (~2.7 s at that throttle) and painted instantly on the next visit; timings panel populated. Full suite (8 files in `pdftest-itp/` + the Install ITP tests in `pdftest-projects/`) green. `service-worker.js` cache → `utzline-itp-cache-v28`.
